...
class MainWindow(QMainWindow):
    ...
    def createActions(self):
        ...
        self.calc_suma = QAction("sumar", self)
        self.calc_suma.triggered.connect(self.sumar)

        self.calc_media = QAction("media", self)
        self.calc_media.triggered.connect(self.media)

        self.calc_varianza = QAction("desviacion", self)
        self.calc_varianza.triggered.connect(self.desviacion)
    ...
    def contextMenuEvent(self, event):
        ...
        context_menu.addAction(self.calc_suma)
        context_menu.addAction(self.calc_media)
        context_menu.addAction(self.calc_varianza)
    ...
    def sumar(self):
        ...
        current_col = self.table_widget.currentColumn()
        current_row = self.table_widget.currentRow()
        for fila in range (current_row):
            if(self.table_widget.item(fila, current_col) != None):
                if(self.table_widget.item(fila, current_col).text() != ""):
                    suma += float(self.table_widget.item(fila, current_col).text())
        self.table_widget.setItem(current_row, current_col, QTableWidgetItem(str(suma)))
        ...
        for fila in range (current_row):
            if(self.table_widget.item(fila, current_col) != None):
                if(self.table_widget.item(fila, current_col).text() != ""):
                    cantidad += 1
                    suma += float(self.table_widget.item(fila, current_col).text())
        self.table_widget.setItem(current_row, current_col, QTableWidgetItem(str(suma/cantidad)))
    ...
    def desviacion(self):
        ...
        for fila in range (current_row):
            if(self.table_widget.item(fila, current_col) != None):
                if(self.table_widget.item(fila, current_col).text() != ""):
                    num = float(self.table_widget.item(fila, current_col).text())
                    array.append(num)
        desviacion = str(numpy.std(array))
        self.table_widget.setItem(current_row, current_col, QTableWidgetItem(desviacion))
